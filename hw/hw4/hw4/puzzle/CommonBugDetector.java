...
public class CommonBugDetector {
    public static class CommonBugPuzzleState implements WorldState {
        ...
        public Iterable<WorldState> neighbors() {
            switch (name) {
                case('s'): return createWorldStateList(makeListOf('a', 'x'));
                case('a'): return createWorldStateList(makeListOf('b'));
                case('b'): return createWorldStateList(makeListOf('c'));
                case('c'): return createWorldStateList(makeListOf('d'));
                case('d'): return createWorldStateList(makeListOf('e'));
                case('e'): return createWorldStateList(makeListOf('g'));
                case('x'): return createWorldStateList(makeListOf('c'));
                default: return null;
            }
        }

        private static List<Character> makeListOf(char a) {
            ArrayList<Character> al = new ArrayList<>();
            al.add(a);
            return al;
        }

        private static List<Character> makeListOf(char a, char b) {
            ArrayList<Character> al = new ArrayList<>();
            al.add(a);
            al.add(b);
            return al;
